...
        //   `${process.env.NEXT_PUBLIC_HOST}/bapi/member/login`,
...
    // 공식 문서에 따르면, jwt 콜백은 JWT가 생성되거나(e.g sign in 성공),

    // 업데이트(e.g useSessing 등 클라이언트에서 session에 접근하였을 때) 되었을 때 실행된다. 함수의 반환 값은 encrypted 되며, 쿠키에 저장된다.
...
      // 즉 로그아웃 처리를 하면서 인자로 보낸 redirect가 true면 해당 redirect 함수를 실행하면서 return url되는 쪽으로 페이지를 이동시킴 ㅋㅋ
